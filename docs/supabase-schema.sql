...
-- families 表策略
CREATE POLICY "允许任何人查看家庭（通过email和pin登录）" 
ON families FOR SELECT USING (true);

CREATE POLICY "允许创建新家庭" 
ON families FOR INSERT WITH CHECK (true);

-- users 表策略
CREATE POLICY "家庭成员可以查看自己家庭的成员" 
...
-- star_logs 表策略
CREATE POLICY "家庭成员可以查看自己家庭的星星记录" 
ON star_logs FOR SELECT USING (
  family_id IN (
    SELECT family_id FROM users WHERE id = auth.uid()
  )
);

CREATE POLICY "系统可以创建星星记录" 
ON star_logs FOR INSERT WITH CHECK (true);
